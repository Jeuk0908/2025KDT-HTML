...
public class  Citizen {  // TODO (2) : 인터페이스 구현 형태로 변경
    ...
    public void travel(Nation tgNation) {
        ...
        if (screenPassed) {
            System.out.println("[여행 성공]" + this.personName + "이 대상 국가:" + tgNation.getNationName() + "에 여행 입국에 성공했습니다.");
        } else {
            System.err.println("[여행 실패]" + this.personName + "이 대상 국가:" + tgNation.getNationName() + "에 여행 입국이 거부됐습니다.");
        }
    }
